...
(function() {
    ...
    if (url.indexOf("//item.taobao.com/item") != -1 || url.indexOf("//detail.tmall.com/item") != -1 || url.indexOf("//chaoshi.detail.tmall.com/item") != -1 || url.indexOf("//detail.tmall.hk/hk/item") != -1 || url.indexOf("//detail.tmall.hk/item") != -1) {
        ...
        function yhxs(xl_class,yhq,wyh,lsj,ewm,data){
        ...
        $.get(apijiekou+'?id='+QueryString("id")+'&m=shangpin', function(data) {
            ...
            var ewm = '<div id="qrcode" style="position: fixed;bottom: 10px;right:35px;z-index: 9999;"><p class="coupon-time"><b>使用淘宝APP扫码购买此商品</b></p></div>';
            if (url.indexOf('//detail.tmall.') != -1 || url.indexOf('//chaoshi.detail.tmall.') != -1) {
                if ($('div').is('.tm-fcs-panel')) {
                    yhxs('.tm-fcs-panel',yhq,wyh,lsj,ewm,data)
                } else {
                    yhxs(data.tm_class,yhq,wyh,lsj,ewm,data)
                }
            } else {
                if ($('div').is('ul.tb-meta')) {
                    yhxs('ul.tb-meta',yhq,wyh,lsj,ewm,data)
                } else {
                    yhxs(data.tb_class,yhq,wyh,lsj,ewm,data)
                }
            }
        ...
        if (url.indexOf("://item.jd.com/coupons?") < 0) {
            ...
            $.get(apijiekou+'jd/?url='+url, function(data) {
                ...
                if (data.coupon_money) {
                    $('.summary-top').after(yhq + lsj + ewm);
                } else if (location.href.indexOf(data.sign) != -1) {
                    if (data.shortURL) {
                        $('.summary-top').after(wyh + lsj + ewm);
                    } else {
                        $('.summary-top').after(wyh + lsj);
                    }
                } else {
        ...
        $.get(apijiekou+'vip/?url='+url, function(data) {
            var ewm = '<div id="qrcode" style="position: fixed;bottom: 3px;right:39px;z-index: 9999;"><p class="coupon-time"><b>使用微信或唯品会APP扫码购买</b></p></div>';
            if (data.url) {
        ...
        var tmselectorList = [];
        if (url.indexOf("//s.taobao.com/search") != -1 || url.indexOf("//s.taobao.com/list") != -1) {
            tmselectorList.push(".items .item");
            tmselectorList.push(".Content--content--sgSCZ12 .Card--doubleCardWrapper--L2XFE73");
            tmselectorList.push(".Content--content--sgSCZ12 .Card--listCardWrapper--ax09OfT");
            tmselectorList.push(".Content--content--sgSCZ12 .Card--doubleCardWrapperMall--uPmo5Bz");
            tmselectorList.push(".Content--content--sgSCZ12 .Card--listCardWrapperMall--E3jI8L4");
        } else if (url.indexOf("//list.tmall.com/search_product") != -1 || url.indexOf("//list.tmall.com//search_product") != -1 || url.indexOf("//list.tmall.com/coudan/search_product") != -1) {
            tmselectorList.push(".product");
            tmselectorList.push(".chaoshi-recommend-list .chaoshi-recommend-item");
        } else if (url.indexOf("//list.tmall.hk/search_product") != -1) {
            tmselectorList.push("#J_ItemList .product");
        } else if (document.getElementById('J_ShopSearchResult')) {
            tmselectorList.push("#J_ShopSearchResult .item");
        }
